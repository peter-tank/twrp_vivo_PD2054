...
mod="/cache/flash.src"; # fallback /data/local/tmp/nc_shell/flash.src, lost script dynamic loading on no access able place. MUST KEEP IN QUOTED.
log="/tmp/nc_shell_tmp.log"; # forced, changing need service fully reloads. MUST KEEP IN QUOTED.
...
#test "$(getprop recovery.crypto.decrypt.result)" = "success"; then
  test -d "/cache" && {
    logs2dir "/cache/ls_$(rdate +%s).log";
    touch "/tmp/copy_out";
